...
🏠  홈 아이콘      → 메인 대시보드 복귀
⚙️  엔진 아이콘    → 분석 엔진 뷰 전환
📂  CSV 드롭존     → 파일 업로드 (하단 고정)
```

**규칙**
- 화면은 메인 대시보드와 엔진 뷰 두 가지만 존재
- 활성 화면 아이콘 강조
  - 메인: 홈 아이콘 녹색 강조
  - 엔진: 엔진 아이콘 황금색 강조
- CSV 드롭존은 좌측 배너 최하단 고정
- 업로드 성공 시 파일명 일부 표시
- 차트, 알림, 설정 등 기타 아이콘은 배치하지 않는다
...
| 리밸런싱 제안 | insight_result["rebalancing"] |
| 헤지 제안 | insight_result["hedge"] |

**규칙**
- LLM 입력값 행을 항상 상단에 노출한다
- Why now? 블록은 반드시 `[태그명]` 형식의 분석 목적 태그를 포함한다
- 4개 블록 순서 고정 (변경 불가)
